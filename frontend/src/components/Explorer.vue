...
const handleImportDataset = async () => {
  ...
  input.onchange = async (e: any) => {
    ...
    try {
      ...
      ElMessage.success('Dataset imported successfully')
      fetchData()
    } catch (err: any) {
...
const handleDeleteDataset = async (id: number) => {
  try {
    await ElMessageBox.confirm('Are you sure you want to delete this dataset?', 'Warning', { type: 'warning' })
    loading.value = true
...
const handleImportModel = () => {
  ...
  input.onchange = async (e: any) => {
    ...
    try {
      ...
      ElMessage.success('Model imported successfully')
      fetchData()
    } catch (err: any) {
      ElMessage.error('Import failed: ' + err.message)
    } finally {
...
const handleTrainModel = async (dataset?: any) => {
  ...
  try {
    const { value: modelName } = await ElMessageBox.prompt('Please enter a name for the new model', 'Train Model', {
    ...
    if (err !== 'cancel') ElMessage.error('Train failed: ' + err.message)
  } finally {
    loading.value = false
  }
}
...
const handleDeleteModel = async (id: number) => {
  try {
    await ElMessageBox.confirm('Are you sure you want to delete this model?', 'Warning', { type: 'warning' })
    loading.value = true
          ...
          <div v-else class="space-y-3">
            ...
            >
              <div>
                ...
                <div class="text-xs text-gray-400 mt-1">
                  Acc: {{ (m.accuracy * 100).toFixed(1) }}% | F1: {{ (m.f1Score * 100).toFixed(1) }}% | 创建于: {{ new Date(m.createdTime).toLocaleDateString() }}
                </div>
              </div>
              <div>
